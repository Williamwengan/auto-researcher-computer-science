#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build the v1.0 final research plan package.

This stage converts validated ideas into standardized research plans. It does
not run experiments and does not claim a completed demo. The goal is to bridge
the completed idea-generation / repair / evaluation / evidence-verification
loop to the next stage: code generation and real experiment execution.
"""
...
@dataclass
class FinalResearchPlan:
    plan_id: str
    task_name: str
    task_spec: str
    research_problem: str
    baseline_weakness: List[str]
    paper_evidence: List[str]
    final_idea: str
    core_hypothesis: str
    method_overview: List[str]
    minimal_new_module: str
    ...
    success_thresholds: List[str]
    failure_criteria: List[str]
    implementation_artifacts: List[str]
    risk_and_mitigation: List[str]
...
def final_research_plans() -> List[FinalResearchPlan]:
    return [
        FinalResearchPlan(
            plan_id="plan_01_physical_property",
            task_name="物理属性预测",
            task_spec="focused_workflow/tasks/benchmark_cv/01_physical_property_prediction.yaml",
            research_problem=(
                "从单张 2D 室内场景图像中，为每个可见物体预测 density、Young's modulus、"
                "Poisson ratio、hardness、friction coefficient 等物理属性，并输出不确定性和失败警告。"
            ),
            baseline_weakness=[
            ...
            final_idea=(
                "Mechanism-consistent physical property prediction plan：保留 Idea 1 的 calibrated interval mapper，"
                "将 Idea 2 修复为 localized material evidence verifier，将 Idea 3 修复为 proposal uncertainty propagation。"
            ),
            core_hypothesis=(
                "如果物理属性预测不直接输出单点值，而是把 object mask、category、material evidence、"
                "property table 和 uncertainty propagation 组合成 calibrated intervals，则可以降低虚假精确性，"
                "并通过 selective prediction 改善可靠性。"
            ),
            method_overview=[
            ...
            minimal_new_module="localized material evidence verifier + calibrated property interval mapper + proposal uncertainty propagation",
            experiment_plan=[
                "构建 indoor_property_manifest.jsonl，包含 image id、object mask、category、material candidates 和 proxy interval labels。",
                "实现 build_material_property_table.py，整理材料属性区间及证据来源。",
                "实现 predict_property_intervals.py，输出 calibrated interval、confidence 和 abstention decision。",
                "运行 category-only prior、CLIP/VLM material baseline 和 shuffled-table baseline。",
                "汇总 density_log_mae、coverage、calibration_error 和 selective_risk。",
            ],
            datasets=[
                "ObjectFolder",
                "ObjectFolder2.0",
            ...
            evidence_verification_status="v0.7 repaired evidence cards: pass rate 1.0, unsupported 0。",
            judge_summary="v2 mechanism-consistent repair: 6 reviewers, 18/18 after wins, win rate 1.0。",
            current_boundary="当前完成最终研究方案生成；尚未执行真实训练/评测。",
            next_execution_step="实现 material-property table 与 interval prediction MVP，并跑 proxy-label calibration experiment。",
        ),
        FinalResearchPlan(
            plan_id="plan_03_indoor3d",
            task_name="室内单图 3D 场景生成",
            task_spec="focused_workflow/tasks/benchmark_cv/03_indoor_scene_generation.yaml",
            research_problem=(
                "从单张室内 RGB 图像推断 room layout、visible objects、occluded hypotheses、3D positions、"
                "geometry proxies、relations、materials/textures 和 renderable 3D scene representation。"
            ),
            baseline_weakness=[
            ...
            final_idea=(
                "Evidence-grounded single-image 3D scene hypothesis planner：用 layout/depth/reconstruction baselines 生成候选场景，"
                "再用 scene-graph relation、support/collision checker 和 uncertainty annotation 修复几何与物理不一致。"
            ),
            core_hypothesis=(
                "如果把单图 3D 生成拆成 visible evidence、layout/depth priors、scene graph hypotheses、"
                "physics/relation verification 和 uncertainty reporting，则可以比单纯生成式扩展更可检验、更适合下游任务。"
            ),
            method_overview=[
            ...
            minimal_new_module="scene-graph hypothesis verifier with support/collision checks and uncertainty reporting",
            experiment_plan=[
                "构建 indoor3d_scene_manifest.jsonl，记录 input image、layout cues、visible objects 和 available GT/proxy labels。",
                "实现 build_scene_graph_hypotheses.py，生成 layout/object/relation candidates。",
                "实现 verify_scene_geometry.py，检查 support relation、collision、out-of-room 和 occlusion consistency。",
                "对比 Text2Room/SceneScape/WonderJourney、depth-only、DUSt3R/MASt3R-style baselines。",
                "汇总 layout_iou、object_count_accuracy、relation_accuracy、collision_rate 和 failure_detection_auc。",
            ],
            datasets=[
                "3D-FRONT",
                "Matterport3D",
            ...
            implementation_artifacts=[
                ...
                "scripts/build_scene_graph_hypotheses.py",
                "scripts/verify_scene_geometry.py",
            ...
            evidence_verification_status="v0.7 evidence-card repair: pass rate 1.0, unsupported 0；使用 seeded evidence bank。",
            judge_summary="v0.6 blind A/B: 3 reviewers, 9/9 after wins, win rate 1.0；Si-style benchmark after win rate 80%。",
            current_boundary="当前完成 idea-generation 方案级验证；尚未实现完整 3D 系统。",
            next_execution_step="实现 scene graph hypothesis verifier 的轻量 MVP，并用公开数据做 proxy consistency evaluation。",
        ),
        FinalResearchPlan(
            plan_id="plan_05_iad_agent",
            task_name="工业异常检测 IAD + Agent",
            task_spec="focused_workflow/tasks/benchmark_cv/05_iad_agent_workflow.yaml",
            research_problem=(
                "为工业异常检测构建 agentic inspection workflow，使系统能协调 normal reference retrieval、"
                "defect localization、self-checking、evidence-grounded report 和 human escalation。"
            ),
            baseline_weakness=[
            ...
            final_idea=(
                "Evidence-Grounded Reference-Consistency IAD Agent：以 normal reference retrieval 为核心，"
                "结合 anomaly heatmap、cross-model disagreement、region-reference consistency score、report checker 和 escalation policy。"
            ),
            core_hypothesis=(
                "如果 defect claim 必须同时绑定 anomaly region、normal reference contrast、model disagreement 和 evidence-grounded report check，"
                "则可以降低由 texture/lighting/reference shift 导致的 false alarms，并提高报告可信度。"
            ),
            method_overview=[
            ...
            minimal_new_module="reference-consistency scoring + evidence-grounded report checker + selective escalation policy",
            experiment_plan=[
                "构建 iad_reference_manifest.jsonl，包含 train/val/test product split 和 normal reference provenance。",
                "实现 build_reference_bank.py，存储 patch embeddings、reference image ids 和 retrieval ranks。",
                "实现 score_reference_consistency.py，计算 anomaly/reference/disagreement/evidence 综合分。",
                "运行 PatchCore、PaDiM、WinCLIP、AnomalyCLIP 等 baseline。",
                "运行 negative controls：random retrieval、shuffled provenance、unverified report 和 contaminated normal bank。",
            ],
            datasets=[
                "MVTec AD",
                "VisA",
            ...
            implementation_artifacts=[
                ...
                "results/iad_agent_results.csv",
                "results/iad_evidence_grounding_audit.csv",
            ],
            risk_and_mitigation=[
            ...
            evidence_verification_status="v0.7 pass rate 0.857, unsupported 0, manual 3。",
            judge_summary="v0.6 blind A/B: 3 reviewers, 7/9 after wins, win rate 0.778；Si-style benchmark after win rate 60%。",
            current_boundary="当前完成研究方案生成；尚未执行 IAD agent 的真实 benchmark 实验。",
            next_execution_step="实现 reference bank 和 reference consistency score 的最小实验，并在 MVTec AD/VisA 子集上验证。",
        ),
    ]


FINAL_RESEARCH_PLAN_SCHEMA: Dict[str, Any] = {
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "title": "FinalResearchPlan",
    "type": "object",
    "required": [
        "plan_id",
        "task_name",
    ...
    "properties": {
        ...
        "baseline_weakness": {"type": "array", "items": {"type": "string"}},
        "paper_evidence": {"type": "array", "items": {"type": "string"}},
        "final_idea": {"type": "string"},
        "core_hypothesis": {"type": "string"},
        "method_overview": {"type": "array", "items": {"type": "string"}},
        "minimal_new_module": {"type": "string"},
        ...
        "implementation_artifacts": {"type": "array", "items": {"type": "string"}},
        "risk_and_mitigation": {"type": "array", "items": {"type": "string"}},
        "evidence_verification_status": {"type": "string"},
        "judge_summary": {"type": "string"},
        "current_boundary": {"type": "string"},
        "next_execution_step": {"type": "string"},
    },
...
def plan_section(plan: FinalResearchPlan) -> str:
    return f"""### {plan.plan_id}: {plan.task_name}
...
{bullet_list(plan.baseline_weakness)}

### 论文证据与校验状态

{bullet_list(plan.paper_evidence)}

证据校验：{plan.evidence_verification_status}
...
> {plan.core_hypothesis}

### 方法概述

{numbered_list(plan.method_overview)}

最小新增模块：

```text
{plan.minimal_new_module}
```

### 实验计划

{numbered_list(plan.experiment_plan)}

### 数据集、Baseline 与指标

{md_table(["类型", "内容"], [
    ["Datasets", ", ".join(plan.datasets)],
...
{bullet_list(plan.ablations)}

负控制：

{bullet_list(plan.negative_controls)}

成功标准：

{bullet_list(plan.success_thresholds)}

失败标准：

{bullet_list(plan.failure_criteria)}

### 实现产物

{bullet_list(plan.implementation_artifacts)}

### 风险与缓解

{bullet_list(plan.risk_and_mitigation)}

### Judge 结果与当前边界

Judge summary：{plan.judge_summary}

当前边界：{plan.current_boundary}

下一步执行：{plan.next_execution_step}
"""


def generate_package_report(plans: List[FinalResearchPlan]) -> str:
    ...
    overview_rows = [
        [
            ...
            plan.task_name,
            plan.final_idea,
            plan.evidence_verification_status,
            plan.judge_summary,
        ]
        for plan in plans
    ]

    return f"""# V1.0 Final Research Plan Package

生成时间：{generated_at}

生成脚本：`focused_workflow/scripts/build_v10_final_research_plan_package.py`
...
```text
idea generation + repair + evaluation + evidence verification
```

但还没有进入：

```text
最终研究方案执行 / 真实实验实现 / demo 系统化交付
```

因此 V1.0 的作用不是继续跑 judge，也不是做 demo，而是把经过生成、修复、盲评和证据校验的 idea 转换成标准化、可追踪、可执行的最终研究方案。它是从“idea 质量已验证”走向“后续可以写代码和跑实验”的中间层。

## 本阶段输出

本阶段生成 3 个 final research plans：

{md_table(["Plan ID", "任务", "最终 idea", "证据状态", "Judge 状态"], overview_rows)}

## 标准研究方案 Schema

每个 final research plan 必须包含以下字段：

{bullet_list(FINAL_RESEARCH_PLAN_SCHEMA["required"])}

这样做的原因是：后续自动代码生成、实验执行、结果表格生成和最终科研报告，都需要稳定字段。如果最终方案只是自然语言，就很难自动衔接到下一阶段。
...
但仍然不能说已经完成端到端科研自动化。更准确的表述是：

> 本项目已完成从科研任务输入到 evidence-grounded idea generation、targeted repair、multi-LLM blind evaluation、reference claim verification 和 final research plan generation 的核心闭环。下一阶段将从“方案生成”推进到“方案执行”，即自动生成实验代码、运行实验、收集结果并形成最终科研报告。
...
```text
Experiment Execution Planning / 实验执行规划
```

它需要把本报告中的 implementation_artifacts 拆成真实代码任务、数据准备任务、运行命令、预期输出表格和验收条件。
"""


def generate_stage_status() -> str:
    return f"""# Workflow Stage Status

生成时间：{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}

本表用于回答“目前我们在哪个阶段”。它刻意区分 idea-generation 闭环、最终研究方案生成、真实实验执行和 demo 系统化交付，避免夸大当前进度。

{md_table(["Step", "阶段", "状态", "说明"], WORKFLOW_STAGE_STATUS)}
...
def main() -> None:
    ...
    plans = final_research_plans()
    package_path = out_dir / "V10_FINAL_RESEARCH_PLAN_PACKAGE_CN.md"
    schema_path = out_dir / "FINAL_RESEARCH_PLAN_SCHEMA.json"
    status_path = out_dir / "WORKFLOW_STAGE_STATUS_CN.md"
    json_path = out_dir / "V10_FINAL_RESEARCH_PLAN_PACKAGE.json"

    package_path.write_text(generate_package_report(plans), encoding="utf-8")
    write_json(schema_path, FINAL_RESEARCH_PLAN_SCHEMA)
    status_path.write_text(generate_stage_status(), encoding="utf-8")
    write_json(
        json_path,
        {
            "version": "v1.0",
            "purpose": "final_research_plan_package",
            "plans": [asdict(plan) for plan in plans],
            "workflow_stage_status": WORKFLOW_STAGE_STATUS,
            "boundary": "No real experiment execution or demo is claimed in v1.0.",
        },
    )

    print("V1.0 final research plan package written:")
